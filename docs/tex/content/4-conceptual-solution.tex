...
\todo{formally proof containments for relationships, e.g., mathe stuff}
...
The class \inlinecode{Movie} contains two items, on the one hand a title as a string and on the other hand a description also as a string. The \inlinecode{MoveScreening} class contains two attributes according to the requirements. The attribute finished as a Boolean, indicates whether the film has finished screening and the attribute name as a string, identifies the name of each film screening. Between the classes \inlinecode{Movie} and \inlinecode{MovieScreening} there is an association as Containment. This association can be understood as a containment, as it is not sensible to change the film in a film performance. In addition, the film should be clearly identifiable through the screening and, at the same time, every screening needs a film. 
Furthermore, the \inlinecode{CinemaHall} class is required. It only has one attribute, namely whether the hall is available to be used for movie screenings. There is an association as containment within the \inlinecode{CinemaHall} and \inlinecode{MovieScreening} classes. This association is a containment because the screening cannot exist without a hall. Additionally, there should only be one hall per screening. The hall can be clearly identified by a screening.
Next, a class called \inlinecode{SeatRow} and another class \inlinecode{Seat} are necessary. The \inlinecode{CinemaHall} class and the \inlinecode{SeatRow} class have an association as a containment.
...
There should also be a containment association from \inlinecode{SeatRow} to \inlinecode{Seat}. This is a containment because a SeatRow has several Seats and a Seat cannot exist without a SeatRow. A seat should be uniquely identifiable in a SeatRow.
In addition, an abstract class \inlinecode{BoookingState} is needed for the booking system. This class is specified by the classes \inlinecode{Reservation} and \inlinecode{Booking}. There is again a containment association from the class to \inlinecode{Seat} to the abstract class \inlinecode{BookingState}. This association is a containment because a BookingState cannot exist without a Seat. A BookingState uniquely identifies a Seat.
\inlinecode{MovieScreening} has a second association to \inlinecode{BookingState}, this one is also a containment. This is a containment because a MovieScreening exists for every BookingState. Thereby, a BookingState cannot exist without a MovieScreening.
In addition, there is to be an abstract class \inlinecode{PriceCategory}, which is specified by three classes. This class is to express in which category the place is booked. For example, in the Parquet, the Lodge or the Lodge with Service. The Singelton classes \inlinecode{PriceCategoryBox}, \inlinecode{PriceCategoryServiceBox} and \inlinecode{PriceCategoryStalls} specify the class \inlinecode{PriceCategory} and have the function mentioned previously. The seat row has an association to the price category in order to assign the respective row to the specific category. This association is not a conatinment as it should be possible to change the PriceCategory of a SeatRow at any time. 
In addition, the customer of the cinema was described in the \inlinecode{Customer} class. The class has three attributes, the first name, the last name and the e-mail address of the customer. The \inlinecode{Customer} class has a containment association with the \inlinecode{BookingState} class in order to be able to assign its own bookings to the customers. This is a Conatinment because a BookingState could not exist without a Customer. A BookingState uniquely identifies a Customer and a Customer has a list of BookingStates.

\subsubsection{Compliance with MPS}
\label{sec:cs-data-mps}

\todo{@Manu: vllt sollten wir den mathe-stuff mit dem text von Jana da oben irgendwie kombinieren. Das ist aktuell so ein riesiger text block der einfach alles erschlägt.}

\todo{How do we need to change the model to comply with MPS? (Hint: enums and singletons)}
\todo{sooo done. musst halt eigentlich nur beschreiben, was sich ändert von dem modell oben, zu dem hier unten, das mit dem enum/singleton kram halt und noch dazu sagen WARUM enums nicht anders gehen. vllt gibts dazu im internet oder in seinem skript was, halt als quelle :)}
