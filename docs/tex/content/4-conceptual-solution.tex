...
\section{Project Infrastructure}\label{sec:infrastructure}\sectionauthor{Frederik Höft}
...
\subsection{Data Model Design}\label{sec:cs-data-model}\subsectionauthor{Jana Schlüsche}
...
The class \inlinecode{Movie} contains two items, on the one hand a title as a string and on the other hand a description also as a string. The \inlinecode{MoveScreening} class contains two attributes according to the requirements. The attribute finished as a Boolean, indicates whether the film has finished screening and the attribute name as a string, identifies the name of each film screening. Between the classes \inlinecode{Movie} and \inlinecode{MovieScreening} there is an association as Containment. Furthermore, the \inlinecode{CinemaHall} class is required. It only has one attribute, namely whether the hall is available to be used for movie screenings. There is an association as containment within the \inlinecode{CinemaHall} and \inlinecode{MovieScreening} classes. Next, a class called \inlinecode{SeatRow} and another class \inlinecode{Seat} are necessary. The \inlinecode{CinemaHall} class and the \inlinecode{SeatRow} class have an association as a containment. There should also be a containment association from \inlinecode{SeatRow} to \inlinecode{Seat}. In addition, an abstract class \inlinecode{BoookingState} is needed for the booking system. This class is specified by the classes \inlinecode{Reservation} and \inlinecode{Booking}. There is again a containment association from the class to \inlinecode{Seat} to the abstract class \inlinecode{BookingState}. \inlinecode{MovieScreening} has a second association to \inlinecode{BookingState}, this one is also a containment. 
In addition, there is to be an abstract class \inlinecode{PriceCategory}, which is specified by three classes. This class is to express in which category the place is booked. For example, in the Parquet, the Lodge or the Lodge with Service. The Singelton classes \inlinecode{PriceCategoryBox}, \inlinecode{PriceCategoryServiceBox} and \inlinecode{PriceCategoryStalls} specify the class \inlinecode{PriceCategory} and have the function mentioned previously. The seat row has an association to the price category in order to assign the respective row to the specific category. In addition, the customer of the cinema was described in the \inlinecode{Customer} class. The class has three attributes, the first name, the last name and the e-mail address of the customer. The \inlinecode{Customer} class has a containment association with the \inlinecode{BookingState} class in order to be able to assign its own bookings to the customers.
...
\subsection{REST API}\label{sec:cs-api}\subsectionauthor{Frederik Höft}
...
\subsection{Isolation Service}\label{sec:cs-isolation}\subsectionauthor{Frederik Höft}
...
\section{Client}\label{sec:cs-client}\sectionauthor{Frederik Höft}% :P
